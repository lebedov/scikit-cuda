...
print "Lets test if the first two resulting eigenvectors (principal components) are orthogonal, by dotting them and seeing if it is about zero, then we can see the amount of the origial variance explained by just two of the 100 dimensions.\n\n\n"
...
for i in range(len(demo_types)):
	...
	print "The dot product of the two ", precisions[i], "precision eigenvectors is:", dot_product
	...
	print "We explained ", 100 * np.sum(std_vec[:2]) / np.sum(std_vec), "% of the variance with 2 principal components in", precisions[i], "precision\n\n"
