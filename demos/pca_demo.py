...
print("Lets test if the first two resulting eigenvectors (principal components) are orthogonal,"
      ...
      " variance explained by just two of the original 100 dimensions.\n\n\n")
...
for i in range(len(demo_types)):
    ...
    X = np.random.rand(1000,100).astype(demo_type) 	
    
    # note that order="F" or a transpose is necessary. 
    # fit_transform requires row-major matrices, and column-major is the default
    X_gpu = gpuarray.GPUArray((1000,100), demo_type, order="F")
    ...
    print("The dot product of the two " + str(precisions[i]) + 
